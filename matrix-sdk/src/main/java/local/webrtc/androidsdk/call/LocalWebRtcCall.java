...
import org.matrix.androidsdk.call.MXWebRtcView;
...
public class LocalWebRtcCall extends LocalCall {
    private static final String LOG_TAG = LocalWebRtcCall.class.getSimpleName();
     ...
     */
    public LocalWebRtcCall(ILocalSession session, Context context, JsonElement turnServer) {
    ...
    private void sendInvite(final SessionDescription sessionDescription) {
        ...
        Event event = new Event(Event.EVENT_TYPE_CALL_INVITE, inviteContent, mSession.getCredentialsUserId(), mCallSignalingRoom.getRoomId());
    ...
    private void sendAnswer(final SessionDescription sessionDescription) {
        ...
        Event event = new Event(Event.EVENT_TYPE_CALL_ANSWER, answerContent, mSession.getCredentialsUserId(), mCallSignalingRoom.getRoomId());
        mPendingEvents.add(event);
        sendNextEvent();
                        ...
                        mUIThreadHandler.post(new Runnable() {
                            ...
                            public void run() {
                                if (!isCallEnded()) {
                                    ...
                                    if (addIt) {
                                        Event event = new Event(Event.EVENT_TYPE_CALL_CANDIDATES, content, mSession.getCredentialsUserId(),
